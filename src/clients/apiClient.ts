...
const login =
  async (username: string, password: string): Promise<TokenData> => {
    ...
    if (response.status !== 200) {
      ...
      throw new Error('Logowanie nieudane, błąd: ' + (errorData.error ?? 'nieznany'))
    }
...
const validateTicketAndGetExitTime =
  async (ticketId: string, apiToken: string): Promise<Date> => {
